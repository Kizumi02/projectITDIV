...
-- jawaban
-- nomor 1
select concat(FirstName,' ',LastName) as Name , CONCAT (Address,',',City) as CustAddress from MsCustomer ORDER BY  DOB asc
-- nomor 2
select concat (right(StaffID,3),'-',LastName) as Staff, Email,Gender from MsStaff WHERE Salary > 1600000
-- nomor 3
CREATE VIEW vw_Q3OrderList as (
select concat(FirstName, ' ', LastName) as CustomerName, convert(varchar(30),OrderDate,105) as OrderDate, RentalDuration 
as CustomerName from MsCustomer cust
INNER JOIN TrOrder ord
ON cust.CustomerID = ord.CustomerID
INNER JOIN TrOrderDetail det
...
-- nomor 4
select concat(Genre) as Title , concat (ReleaseDate':'Msfilms)as Film Details
as CustomerName from MsCustomer cust
INNER JOIN TrOrder ord
ON cust.CustomerID = ord.CustomerID
INNER JOIN TrOrderDetail det
ON ord.OrderID = det.OrderID
-- nomor 5
select concat(FirstName, ' ', Genre) as CustomerName, convert(varchar(30),OrderDate,105) as OrderDate, RentalDuration 
as CustomerName from MsCustomer cust
INNER JOIN TrOrder ord
ON cust.CustomerID = ord.CustomerID
INNER JOIN TrOrderDetail det
